...
class _AdminMapPageState extends State<AdminMapPage> {
  ...
  Future<void> _refreshMarkers(List<PatientModel> patients, dynamic faskesProfile) async {
        ...
        if (_expandedClusterKey == entry.key) {
          ...
          for (int i = 0; i < count; i++) {
            ...
            Color color = p.status.toLowerCase().contains('sembuh')
                ? Colors.green
                : (p.status.toLowerCase().contains('drop') ? Colors.red : const Color(0xFF1060EF));
    ...
    // --- MARKER KHUSUS UNTUK RUMAH SAKIT / FASKES ---
      ...
      body: Consumer2<PatientProvider, FaskesProfileProvider>(
        builder: (context, patientProv, faskesProv, child) {
          ...
          if (_lastDataHash != currentHash) {
            ...
            _refreshMarkers(filteredPatients, profile); 
          }
                ...
                child: Column(
                  children: [
                    FloatingActionButton.small(
                      ...
                      backgroundColor: Colors.white,
                      onPressed: () {
                        // --- KODE GEOLOCATOR SUDAH DIHAPUS 100% ---
                        // Sekarang murni menggunakan kordinat Profil Faskes
                        if (profile != null && profile.latitude != null && profile.longitude != null) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Kembali ke lokasi Faskes...'),
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text('Lokasi Faskes belum diatur di Profil.'),
